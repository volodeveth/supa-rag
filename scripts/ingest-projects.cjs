...
const DOCS_DIR = path.resolve(__dirname, "project-docs");
...
async function deleteOldProjectDocs() {
  ...
  if (!res.ok) {
    console.warn(`Delete warning: ${res.status}`);
    return 0;
  }
...
async function main() {
  ...
  // Delete old project docs (also delete test entries)
  ...
  await fetch(
    `${SUPABASE_URL}/rest/v1/documents?metadata->>source=eq.test2`,
    {
      method: "DELETE",
      headers: {
        apikey: SUPABASE_KEY,
